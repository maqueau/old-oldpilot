...
fn copy_to_clipboard(text: &str) -> Result<()> {
    ...
    for tool in ["pbcopy", "wl-copy", "xclip"] { let mut cmd = Command::new(tool); if tool == "xclip" { cmd.args(["-selection", "clipboard"]); } if let Ok(mut child) = cmd.stdin(Stdio::piped()).spawn() { if let Some(stdin) = child.stdin.as_mut() { stdin.write_all(text.as_bytes())?; } if child.wait()?.success() { return Ok(()) } } }
...
fn run_cs(parts: Vec<String>, model: Option<&str>) -> Result<()> {
    ...
    match action_picker(&suggestion)? { 'e' => if confirm_execute(&c)? { let shell = resolve_shell(&c).ok_or_else(|| anyhow!("Configured shell `{}` was not found", shell_name(&c)))?; let status = Command::new(shell).arg("-c").arg(&suggestion).status()?; if !status.success() { return Err(anyhow!("command exited with status {:?}", status.code())); } } else { println!("Canceled."); }, 'c' => { copy_to_clipboard(&suggestion)?; println!("Copied to clipboard."); }, 'x' => run_ce(vec![format!("Explain this command: {}", suggestion)], model)?, _ => println!("Canceled.") }
    Ok(())
}

fn main() -> Result<()> { let cli = Cli::parse(); match cli.command { Commands::Ce { prompt } => { let c = load_config().unwrap_or_default(); let m = default_model(&c); run_ce(prompt, Some(&m)) }, Commands::Cex { prompt } => run_ce(prompt, None), Commands::Cs { prompt } => { let c = load_config().unwrap_or_default(); let m = default_model(&c); run_cs(prompt, Some(&m)) }, Commands::Csx { prompt } => run_cs(prompt, None), Commands::Doctor => run_doctor(), Commands::Settings => { println!("Use the config file at {}", config_path()?.display()); Ok(()) } } }
